...
void CGhost::Update( float dt )
{
	...
	if ( gPlayer )
	{
		...
		if ( mAction == 1 )
		{
			if ( pos.y > gPlayer->GetPosition().y + PLAYER_HEIGHT - 2.0f )
			{
				pos.y -= ( gPlayer->GetPosition().y + PLAYER_HEIGHT - 2.0f - pos.y ) * dt / 2.0f;
		...
		if ( mAction == 0 && distanceToPlayer < 16.0f * 16.0f )
			mAction = 1;
		if ( mAction == 1 && distanceToPlayer < 1.5f*1.5f && pos.y <= gPlayer->GetPosition().y + PLAYER_HEIGHT )
			mAction = 2;
...
//			force = force * BAT_DIVE_SPEED / 3.0f;
		...
		if ( mAction == 2 && pos.y > gPlayer->GetPosition().y + PLAYER_HEIGHT )
			force = force * 2.0f;

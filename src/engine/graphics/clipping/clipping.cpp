...
//#endif
		...
		// Transformed frustum.
		ScePspFMatrix4		model_view_matrix;		
		...
		static inline void calculate_frustum(const ScePspFMatrix4& clip, frustum_t* frustum)
		{
			...
			// build the near regardless of if we are clipping near
			// so that we can use it as an early out
//#if CLIP_NEAR
...
//#endif
		...
		void begin_frame()
		{
			glGetFloatv(GL_PROJECTION_MATRIX, (GLfloat*)&projection_view_matrix);
...
//			ScePspFMatrix4	model_matrix;
			glGetFloatv(GL_MODELVIEW_MATRIX, (GLfloat*)&model_view_matrix);
//			sceGumStoreMatrix(&model_matrix);

			// Combine the matrices (multiply projection-view by model).
			//ScePspFMatrix4	projection_view_model_matrix;
			multiply(model_view_matrix, projection_view_matrix, &projection_view_model_matrix);
		...
		{
			const plane_type* const	last_plane	= &clipping_frustum[CLIP_NUM];
			...
			for (const plane_type* plane = &clipping_frustum[0]; plane != last_plane; ++plane)
			{
				...
				{
					// Is this one outside the plane?
					if (!point_in_front_of_plane(vertex->xyz, *plane))
					{
						// Then the poly needs clipped.
						return true;
					}
				}
			}
		...
		void begin_frame()
		{
			...
			// Combine the two matrices (multiply projection by view).
			//multiply(view, proj, &projection_view_matrix);
...
//			sceGumStoreMatrix(&model_matrix);
			ScePspFMatrix4	model_matrix;
			glGetFloatv(GL_MODELVIEW_MATRIX, (GLfloat*)&model_matrix);

			// Combine the matrices (multiply projection-view by model).
			ScePspFMatrix4	projection_view_model_matrix;
			multiply(model_matrix, projection_view_matrix, &projection_view_model_matrix);
...
{
	return (point_in_front_of_plane(v1, clipping_frustum[plane_index_near]) ||
point_in_front_of_plane(v2, clipping_frustum[plane_index_near]) ||
point_in_front_of_plane(v3, clipping_frustum[plane_index_near])) &&
	(point_in_front_of_plane(v1, clipping_frustum[plane_index_far]) ||
point_in_front_of_plane(v2, clipping_frustum[plane_index_far]) ||
point_in_front_of_plane(v3, clipping_frustum[plane_index_far]));
}

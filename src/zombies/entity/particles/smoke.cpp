...
{
	mTimer = 0.0f;
	mEmissionRate = 300.0f;
	mParticlesLeft = -1;
	mX = x;
	mZ = z;
	mY = y;
	mTexture = CManagedTexture::Load( &gResourceManager, "particle.bmp" );
}
...
{
	CParticle* p = new CParticle( 1.9f, 
		mX + (rand()%1000)/40.0f - 12.5f, (rand()%100)/50.0f + mY, mZ + (rand()%1000)/40.0f - 12.5f, 2.0f, 3.0f );

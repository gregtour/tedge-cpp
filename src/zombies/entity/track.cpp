...
//#include "../../engine/graphics/TexFont3.h"


//const char*	LEVEL_GROUND_TEXTURE = "grass.jpg";
const char* LEVEL_GROUND_TEXTURE = "desert-tile.jpg";
...
CTrack::CTrack()
{
	...
	gWorld->Add(mTrackModel->GetVerts(), GROUND_OBJ_TYPE, 1|4);

    SBoundingBox bounds;
    bounds.min = SVector3(0, -100, 0);
    bounds.max = SVector3(1000, 100, 1000);

    gWorld->SetBoundries(bounds);
    //gWorld->SortStaticObjects(20, 4, 20);
    gWorld->SortStaticObjects(150, 10, 150);
...
void CTrack::BuildTrack()
    ...
    for (int x = 0; x < 100; x++)
        for (int y = 0; y < 100; y++)
            heights[x][y] = sinf(x/5.0f) * cosf(y/5.0f) * 3.0f;
            //heights[x][y] = (x + y) * 0.02f + (y-50)*(y-50)*0.03f + 10.0f/((x-50)*(x-50)+1.0f);

    I3DVERTBUF* buf = new I3DVERTBUF(false, I3D_VC_XYZ | I3D_VC_NORMAL | I3D_VC_UV1, 99*99*2*3 + sizeof(TRAIN_MODEL)/sizeof(double)/3);
    buf->Lock();
    for (int x = 0; x < 99; x++)
        for (int y = 0; y < 99; y++)
            {
            ...
            buf->SetNormal(k.x, k.y, k.z);
            buf->SetUV0(x*0.2+1*0.2, y*0.2+1*0.2);
            buf->NextVert();
    ...
    for (int i = 0; i < sizeof(TRAIN_MODEL)/sizeof(double); i += 3)
    {
        int k = (int)(i/9);
        SVector3 a(TRAIN_MODEL[9*k], TRAIN_MODEL[9*k+1], TRAIN_MODEL[9*k+2]);
        SVector3 b(TRAIN_MODEL[9*k+3], TRAIN_MODEL[9*k+4], TRAIN_MODEL[9*k+5]);
        SVector3 c(TRAIN_MODEL[9*k+6], TRAIN_MODEL[9*k+7], TRAIN_MODEL[9*k+8]);
        SVector3 normal = CrossProduct(a - b, a - c);
        normal.Normalize();

        buf->SetPos(TRAIN_MODEL[i], TRAIN_MODEL[i+1], TRAIN_MODEL[i+2]);
        buf->SetNormal(normal.x, normal.y, normal.z);
        buf->SetUV0(TRAIN_MODEL[i], TRAIN_MODEL[i+2]);
        buf->NextVert();
    }
...
void CTrack::Render()
{
	...
	//  font->DrawTextbox(50.0f, 50.0f, 300.0f, L"Testing font rendering support with TEXFONT3 and the built-in console font. This text should appear at the top left hand corner of the screen and should wrap around nicely. Testing.");

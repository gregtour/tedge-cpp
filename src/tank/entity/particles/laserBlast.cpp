...
CLaserBlast::CLaserBlast( SVector3 pos, float r, float g, float b, float size, int num )
{
	mTimer = 0.0f;
...
CLaserBlast::~CLaserBlast()
{
}

void CLaserBlast::CreateParticle()
{
	float x, y, z;
	x = mX;
	y = mY;
	z = mZ;
	CParticle* p = new CParticle( 0.15f, x, y, z, 0.0f, 0.5f );
	...
	y = (rand()%1000/1000.0f) * size - size / 2.0f;
...
void CLaserBlast::EffectParticle( CParticle* p, float dt )
{
	float x, y, z;
	p->GetSpeed( &x, &y, &z );
	x *= (1.0f + 1.2f * dt);
...
void CLaserBlast::Logic( float dt )
{
}
